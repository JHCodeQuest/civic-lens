...
export default function PartiesPage() {
  ...
  return (
    <main className="mx-auto max-w-5xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <nav className="mb-8 flex flex-wrap gap-x-4 gap-y-2" aria-label="Region navigation">
        {grouped.map(({ region }) => (
          <a
            key={region}
            href={`#${region}`}
            className="text-sm text-govuk-blue hover:underline dark:text-govuk-blue"
          >
            {region}
          </a>
        ))}
      ...
      <div className="grid gap-5">
        {grouped.map(({ region, items }) => (
          <section key={region} aria-labelledby={region}>
            <h2
              id={region}
              className="mb-4 mt-8 scroll-mt-20 text-2xl font-bold text-govuk-black first:mt-0 dark:text-white"
            >
