...
export default function Home() {
  return (
    <>
      <section className="border-b border-govuk-border bg-gray-50 px-4 py-12 text-center sm:px-6 sm:py-28">
        <h1 className="mb-4 text-4xl font-bold tracking-tight text-govuk-black sm:text-5xl">
          Understanding UK Politics
        </h1>
        <p className="mx-auto mb-3 max-w-2xl text-lg text-govuk-secondary-text">
          Helping people understand UK politics through simple explanations, data, and
          interactive tools.
        </p>
        <p className="mx-auto mb-8 max-w-xl text-sm text-govuk-secondary-text">
        ...
        <div className="grid gap-6 sm:grid-cols-3">
          {whatItDoes.map(({ title, description, href }) => (
            <Link
              key={title}
              href={href}
              className="rounded-sm border border-govuk-border bg-white p-4 no-underline transition hover:border-l-govuk-blue sm:p-6 border-l-4"
            >
              <h3 className="mb-2 text-lg font-semibold text-govuk-black">{title}</h3>
              <p className="text-sm leading-relaxed text-govuk-secondary-text">{description}</p>
            </Link>
          ))}
        </div>
      </section>

      <section className="border-t border-govuk-border bg-gray-50 px-4 py-12 sm:px-6 sm:py-20">
        <div className="mx-auto max-w-5xl">
          ...
          <div className="grid gap-6 sm:grid-cols-2">
            <div className="rounded-sm border border-govuk-border bg-white p-4 sm:p-6">
