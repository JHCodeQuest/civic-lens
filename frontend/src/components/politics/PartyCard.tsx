...
export default function PartyCard({ party, showRegion = false }: PartyCardProps) {
  const { slug, name, leader, position, summary, region, priorities, founded, color } = party

  return (
    <Link href={`/parties/${slug}`} className="block no-underline">
      <article
        id={slug}
        className="rounded-sm border border-govuk-border bg-white p-4 transition hover:border-l-govuk-blue hover:shadow-md sm:p-5 border-l-4 dark:border-govuk-border dark:bg-[#1a1a1a] dark:hover:border-l-govuk-blue"
